...
Proof using Type.
  ...
  step_symex. case (GetRegFull_R s _ ltac:(eassumption) _ _ _ HSv) as (Heq & v0 & Hv0 & Hnth);
  clear HSv; subst s0; cbv [fst snd] in *. 
...
Proof.
  ...
  - rewrite !le_combine_cons_add.
    ...
    symmetry; apply Z.mod_pow2_add_low; assumption.
Qed.
...
Proof.
  ...
  intro H. Option.inversion_option. subst v. 
...
(* Make frame and G implicit for lemmas used by tactics *)
Arguments App_R {frame G opts descr} _ _ _ _ _ _ _ _ _.
Arguments Merge_R {frame G descr} _ _ _ _ _ _ _ _ _.
  Arguments GetFlag_R {frame G} _ _ _ _ _ _ _.
  Arguments GetRegFull_R {frame G} _ _ _ _ _ _ _.
  Arguments GetReg_R {frame G opts descr} _ _ _ _ _ _ _.
  Arguments Address_R {frame G opts descr} _ _ _ _ _ _ _ _.
  Arguments SetFlag_R {frame G} _ _ _ _ _ _ _ _ _.
  Arguments GetOperand_R {frame G opts descr} _ _ _ _ _ _ _ _ _ _.
  Arguments R_SetOperand {frame G opts descr} _ _ _ _ _ _ _ _ _ _ _ _ _.
  Arguments R_SetReg {frame G opts descr} _ _ _ _ _ _ _ _ _ _.
  Arguments HavocFlags_R {frame G} _ _ _ _ _ _.
  Arguments Load64_R {frame G} _ _ _ _ _ _ _ _ _.
  Arguments Store64_R {frame G} _ _ _ _ _ _ _ _ _ _ _.
  Arguments R_subsumed {frame} _ _ _ _ _ _ _ _.
...
Ltac step_Merge :=
  ...
  | H : Merge ?e ?s = Success (?i, ?s') |- _ =>
    ...
    let t := open_constr:(fun A B => Merge_R s _ A e v B _ _ H) in
    unshelve (edestruct t as (Hs'&Hl&Hi); clear H); shelve_unifiable;
    [eassumption|try solve [repeat (econstructor; eauto)]|..]
  end.
...
Ltac step_SetOperand :=
  match goal with
  | H : @Symbolic.SetOperand _ _ _ _ ?a ?i ?s = Success (?_tt, ?s') |- _ =>
      ...
      case (R_SetOperand s _ ltac:(eassumption) _ _ _ _ _ _ H _ ltac:(eauto 99 with nocore))
        as (m&?Hm&HR&Hl); clear H; try (Option.inversion_option; subst)
  end.
...
Ltac step_bind :=
    ...
    | H : (x <- ?v; ?k)%x86symex ?s = Success _ |- _ =>
      ...
      | match _ with _ => _ end = Success _ =>
        ...
        let HH := fresh "HS" x in
        destruct v as [[x ?]|] eqn:HH in H;
          [cbn [fst snd] in H; try (inversion_ErrorT_step; Prod.inversion_prod; subst)
					|solve[exfalso;clear-H;inversion H]]
      | _ => idtac
      end
    end.
