...
Lemma build_merge_stack_placeholders_ok_R {descr:description} {errules : extra_rewrite_rules_opt} G s s' frame frame' ms
...
Lemma build_merge_base_addresses_ok_R
      {descr:description} {dereference_scalar:bool} {errules : extra_rewrite_rules_opt}
...
Lemma mapM_GetReg_ok_R_full {descr:description} {errules : extra_rewrite_rules_opt} G regs idxs reg_vals s s' frame ms
...
Lemma mapM_GetReg_ok_R {descr:description} {errules : extra_rewrite_rules_opt} G regs idxs s s' frame (ms : machine_state)
      (H : mapM GetReg regs s = Success (idxs, s'))
      (d := s.(dag_state))
      (d' := s'.(dag_state))
      (r := s.(symbolic_reg_state))
...
Lemma SymexLines_ok_R {errules : extra_rewrite_rules_opt} frame G s m asm _tt s'
      (d := s.(dag_state))
      (d' := s'.(dag_state))
      (r := s.(symbolic_reg_state))
      (r' := s'.(symbolic_reg_state))
...
Lemma LoadArray_ok_R {descr:description} {errules : extra_rewrite_rules_opt} frame G s ms s' base base_val base_word_val len idxs
...
Lemma LoadOutputs_ok_R {descr:description} {dereference_scalar:bool} {errules : extra_rewrite_rules_opt} frame G s ms s' outputaddrs output_types output_vals idxs
...
Theorem symex_asm_func_M_correct
        {output_scalars_are_pointers:bool} {errules : extra_rewrite_rules_opt}
