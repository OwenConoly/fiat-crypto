...
Definition SetRegFresh {descr:description} {errules : extra_rewrite_rules} (r : REG) : M idx :=
...
(* light alias *)
Definition App {descr:description} {errules : extra_rewrite_rules} (e : Symbolic.node idx) : symexM idx := fun st => Success (merge (simplify st e) st).
...
Definition compute_array_address {descr:description} {errules : extra_rewrite_rules} (base : idx) (i : nat)
  := (offset <- Symbolic.App (zconst 64%N (8 * Z.of_nat i), nil);
      Symbolic.App (add 64%N, [base; offset]))%x86symex.

Definition build_merge_array_addresses {descr:description} {errules : extra_rewrite_rules} (base : idx) (items : list idx) : M (list idx)
  := mapM (fun '(i, idx) =>
             (addr <- compute_array_address base i;
              (fun s => Success (addr, update_mem_with s (cons (addr,idx)))))
          )%x86symex (List.enumerate items).

Fixpoint build_merge_base_addresses {descr:description} {dereference_scalar:bool} {errules : extra_rewrite_rules} (items : list (idx + list idx)) (reg_available : list REG) : M (list ((REG + idx) + idx))
...
Definition symex_ident {descr:description} {t} {errules : extra_rewrite_rules} (idc : ident t) : symex_T t.
Proof.
...
Fixpoint symex_expr {descr:description} {t} {errules : extra_rewrite_rules} (e : API.expr (var:=var) t) : symex_T t
...
Definition symex_PHOAS_PHOAS {t} {errules : extra_rewrite_rules} (expr : API.Expr t) (input_var_data : type.for_each_lhs_of_arrow _ t) (d : dag)
...
Definition symex_PHOAS
           {t} {errules : extra_rewrite_rules} (expr : API.Expr t)
...
Definition compute_stack_base {descr:description} {errules : extra_rewrite_rules} (stack_size : nat) : M idx
...
Definition build_merge_stack_placeholders {descr:description} {errules : extra_rewrite_rules} (stack_size : nat)
  : M idx
...
Definition LoadArray {descr:description} {errules : extra_rewrite_rules} (base : idx) (len : nat) : M (list idx)
  := mapM (fun i =>
             (addr <- compute_array_address base i;
              Remove64 addr)%x86symex)
          (seq 0 len).

Definition LoadOutputs_internal {descr:description} {dereference_scalar:bool} {errules : extra_rewrite_rules} (outputaddrs : list ((REG + idx) + idx)) (output_types : type_spec)
  : M (list (idx + list idx))
...
Definition LoadOutputs {descr:description} {dereference_scalar:bool} {errules : extra_rewrite_rules} (outputaddrs : list ((REG + idx) + idx)) (output_types : type_spec)
